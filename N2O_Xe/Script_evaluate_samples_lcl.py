...
def read_local(isys):
    ...
    for ir, idcd in enumerate(iruns):
        ...
        nfile_i = os.path.join(
            workdir, 'nhist_{:s}_{:d}.npy'.format(tag, idcd))
        
        # Time file
        tfile_i = os.path.join(
            workdir, 'ntist_{:s}_{:d}.npy'.format(tag, idcd))
        
        if not os.path.exists(nfile_i) or False:
            ...
            for iframe, frame in enumerate(dcd.trajectory):
                ...
                if not iframe%eval_skip:
                    ...
                    # Compute distances
                    dsts_atm = (
                        distance_array(pos_trgt, pos_solv, box=cell)
                        ).reshape(-1)
                    ...
                    # Append frame result
                    nlst.append(nhst.astype(np.int8))
            ...
            # Save results
            np.save(nfile_i, np.array(nlst, dtype=np.int8))
        ...
        for ismpl in Smpls[Ti][Vmi]:
            ...
            if not os.path.exists(nfile) or request_reappending:
                ...
                nfile_0 = os.path.join(
                    workdir, 'nhist_{:s}_{:d}.npy'.format(tag, 0))
...
for it, Ti in enumerate(T):
    ...
    if len(np.array(sys_cnt[sys_trgt]).shape):
        title += r'N$_2$O (center of mass) and '
    else:
        title += r'N$_2$O ({:s}) and '.format(
            sys_atm[sys_solv][sys_cnt[sys_trgt]])
    if len(np.array(sys_cnt[sys_solv]).shape):
        title += r'{:s} (center of mass) '.format(sys_tag[sys_solv])
    else:
        title += r'{:s} ({:s}) and '.format(
            sys_tag[sys_solv], sys_atm[sys_solv][sys_cnt[sys_solv]])
    title += 'at {:.1f} K'.format(float(Ti))
    ...
    plt.savefig(
        os.path.join(
            maindir, rsltdir, 'g_solute_solvent_{:s}.png'.format(sys_solv)),
        format='png', dpi=dpi)
    plt.close()
