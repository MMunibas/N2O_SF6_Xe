...
# Working files
workrun = 'vibration_{:d}.sh'
workinp = 'vibration_{:d}.inp'
workout = 'vibration_{:d}.out'
...
def read_results(idcd):
    
    # Read results
    listfreq = ''
    ivib = 0
    complete = False
    with open(workout.format(idcd), 'r') as fout:
    ...
    # Write results
    rfile = os.path.join(rsltdir, rsltfile.format(idcd))
...
while not done:
    ...
    for idcd, dcdfile in enumerate(dcdfiles):
        ...
        if os.path.exists(workout.format(idcd)):
            
            # Read results
            complete = read_results(idcd)
        ...
        inplines = inplines.replace('FOUT', workout.format(idcd))
        
        # Write run file
        sfile = workrun.format(idcd)
        ...
        while max_cap:
            ...
            for it, tid in enumerate(tskids):
                ...
                if not tid in idslist:
                    ...
                    # Read results
                    complete = read_results(tskdcd[it])
    ...
    rfile_last = os.path.join(rsltdir, rsltfile.format(idcds[-1]))
