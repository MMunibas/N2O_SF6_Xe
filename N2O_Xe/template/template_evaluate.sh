...
for i in $(seq 1 $NPROD); do
    j=$(($i-1))
    stepdone=0
    while [ "$stepdone" -eq 0 ]; do
        diff=$(($NPROD-$i))
        if test -f "vibmode_"$VMODE"_vector.$j.dat"; then
            ...
            srun $my_charmm -i forces.$j.inp -o vibmode_forces.$j.out
            grep "MODE   "$VMODE"  FREQ=" vibmode_forces.$j.out > vibmode_"$VMODE"_forces.$j.dat
            grep -A3 '   EIGENVECTOR:' vibmode_forces.$j.out > vibmode_"$VMODE"_vector.$j.dat
            rm -f forces.$j.inp vibmode_forces.$j.out
            stepdone=1
        elif [ "$diff" -eq 0 ]; then
