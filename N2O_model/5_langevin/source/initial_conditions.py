...
def get_velocities(
    j=None, rot_idx=None, nu_as=None, nu_s=None, nu_d=None, nu_d_idx=None,
    temperature=None):
    ...
    mode_d = np.array(
        [
            [
            [-0.11288,  0.00000,  0.00000],
            [ 0.22033,  0.00000,  0.00000],
            [-0.09408,  0.00000,  0.00000]
            ],
            [
    ...
    # Convert index
    if isinstance(rot_idx, int):
        rot_idx = np.array([rot_idx], dtype=int)
    else:
        rot_idx = np.array(rot_idx, dtype=int)

    # Convert j state
    if isinstance(j, (int, np.int16, np.int32, np.int64)):
        j = np.array([j]*len(rot_idx), dtype=int)
    else:
        j = np.array(j, dtype=int)
        

    # Check length
    if len(j) != len(rot_idx):
        raise IOError(
            "Mismatch in given j state numbers and rotation axis rot_idx")
    ...
    w = np.sqrt(2.*Ej[rot_idx]/Ip[rot_idx])

    # Total angular momentum
    L = np.sqrt(2.*Ej[rot_idx]*Ip[rot_idx])
    ...
    rotation_direction = np.array(
        [np.cross(atom_directions, Ib[idxi]) for idxi in rot_idx])
    ...
    for ii, idxi in enumerate(rot_idx):
    ...
    print()
    print(
        "Atomic symmetric stretch vibrational velocities " 
        + "in Angstrom per akma time unit")
    print(vnu_s_akma)
    print("Respective kinetic energy (kcal / mol) and reference energy")
    ...
    if nu_d_idx is None:
        ...
        mode_di = (
            np.sin(theta)**2*mode_d[0]
            + np.cos(theta)**2*mode_d[1])
        
    else:
        
        mode_di = mode_d[nu_d_idx]
    ...
    if nu_d is None:
        ...
        # Center normal mode distribution
        mode_di -= masses.reshape(-1, 1)*np.sum(mode_di, axis=0)/np.sum(masses)

        # Reference kinetic energy
        Enu_d_mode = np.sum(np.sum(mode_di**2, axis=1)/(2.*masses))

        # Momentum scaling factor 
        pfac = Enu_d/Enu_d_mode

        # Atomic vibrational momentums
        pnu_d = mode_di*np.sqrt(pfac)
        ...
        mode_di -= (
            masses.reshape(-1, 1)
            *np.sum(mode_di, axis=0)/np.sum(masses))
